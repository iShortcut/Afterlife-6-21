...
const Header = () => {
  ...
  const calculateDropdownPosition = useCallback(() => {
    ...
    for (const key in dropdownRefs) {
      ...
      if (ref?.nextElementSibling) {
        ...
        // Use a simple width calculation for robustness
  ...
  const handleClickDropdownButton = (dropdownName: string) => {
    setActiveDropdown(prev => {
      ...
      if (next) {
        // Ensure position is calculated on click as well
  ...
  const renderDropdown = (name: keyof typeof dropdownMenus, buttonContent: React.ReactNode) => (
    <div className="relative" onMouseEnter={() => handleMouseEnterDropdown(name)} onMouseLeave={handleMouseLeaveDropdown}>
      <button ref={dropdownRefs[name]} onClick={() => handleClickDropdownButton(name)} className={`flex items-center gap-1 text-slate-700 hover:text-indigo-700 transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 rounded-md px-2 py-1 whitespace-nowrap text-sm md:text-base ${getActiveDropdownClass(name)}`} aria-expanded={activeDropdown === name} aria-haspopup="true">
        {buttonContent}
        <ChevronDown size={16} className={`transition-transform duration-200 ${getArrowRotationClass(name)}`} />
      </button>
      <AnimatePresence>
        {activeDropdown === name && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className={`absolute mt-2 rounded-md shadow-lg z-10 p-4 flex flex-row flex-wrap gap-4 ${dropdownPositions[name]} ${activeDropdown === name ? 'bg-indigo-50' : 'bg-white'}`} role="menu" aria-orientation="horizontal">
            {dropdownMenus[name].map((item) => (
              item.onClick ? (
                <button key={item.label} onClick={item.onClick} className="flex items-center gap-2 px-3 py-2 text-sm text-slate-700 hover:text-indigo-700 whitespace-nowrap">
    ...
    <header ref={headerRef} className="bg-white shadow-sm sticky top-0 z-50">
      <div className="container mx-auto px-4 py-4 flex items-center justify-start md:gap-x-4 lg:gap-x-12">
        <Link to="/" className="flex items-center gap-2 font-serif tracking-wide text-indigo-900 flex-shrink-0 text-lg md:text-xl" aria-label="Afterlife">
          <BookHeart size={28} className="text-indigo-700" />
          <span className="font-medium">Afterlife</span>
        </Link>
        <div className="flex-grow flex justify-end md:hidden">
          <button onClick={toggleMenu} className="p-2 text-slate-700" aria-label="Toggle menu">
        ...
        <nav className="hidden md:flex items-center flex-grow justify-start gap-4 lg:gap-6 xl:gap-8 min-w-0 flex-wrap">
          ...
          {user && renderDropdown('dashboard', <><Settings size={18} /><span>Dashboard</span></>)}
          ...
          {user ? (
            ...
            <div className="flex items-center gap-2">
              <Link to="/login" className="text-slate-700 hover:text-indigo-700 px-3 py-2 rounded-md text-sm md:text-base">Sign In</Link>
              <Link to="/register" className="px-4 py-2 rounded-md bg-indigo-600 text-white hover:bg-indigo-700 text-sm md:text-base">Create Account</Link>
            </div>
          )}
        </nav>
      </div>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.nav id="mobile-menu" initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ opacity: 0 }} className="md:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {/* Simplified for brevity, vertical layout is standard here */}
            </div>
          </motion.nav>
        )}
      </AnimatePresence>
