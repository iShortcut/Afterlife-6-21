/*
  # Fix Event Media Storage Configuration
  
  1. Changes
    - Add IF NOT EXISTS checks to prevent duplicate policy creation
    - Keep all the same functionality but avoid errors when policies already exist
    - Maintain the same helper functions
  
  2. Security
    - Same RLS policies for event media access
    - Same helper functions for path extraction and access control
*/
...
AS $$
DECLARE
  event_visibility event_visibility_enum;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    AND policyname = 'Allow public read access for event media'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE tablename = 'objects' 
    AND schemaname = 'storage' 
    AND policyname = 'Allow event owner to upload/update their media'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE tablename = 'objects' 
    AND schemaname = 'storage' 
    AND policyname = 'Allow event owner to update their media'
  ) THEN
  ...
  IF NOT EXISTS (
    ...
    WHERE tablename = 'objects' 
    AND schemaname = 'storage' 
    AND policyname = 'Allow event owner to delete their media'
  ) THEN
