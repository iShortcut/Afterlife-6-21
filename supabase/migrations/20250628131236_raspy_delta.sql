...
-- Create unique indexes instead of constraints with NULLS NOT DISTINCT
CREATE UNIQUE INDEX unique_user_invitation ON group_invitations (group_id, user_id) WHERE user_id IS NOT NULL;
CREATE UNIQUE INDEX unique_email_invitation ON group_invitations (group_id, email) WHERE email IS NOT NULL;
